...
| Auth | Supabase JWT via `Authorization: Bearer <token>` header. **Required for write operations** (POST/PATCH/DELETE). GET requests are public. |
...
**Authentication required.** Body schema (`glossWriteSchema`):
...
**Authentication required.** Partial payload (`glossUpdateSchema`). Omitting a relation array leaves it unchanged; providing it replaces the entire set.
...
**Authentication required.** No response body (`204`). Use the companion endpoint below to surface warnings:
...
**Authentication required.** Body schema (`situationWriteSchema`):
...
**Authentication required.** Where `:id` is the situation id (cuid or seed-defined).
...
**Authentication required.** Where `:id` is the situation id.
...
| `401` | Unauthorized - Missing or invalid auth token (write operations only). |
...
See `SUPABASE_AUTH_SETUP.md` in the project root for complete authentication setup instructions.

**Quick summary:**
1. Get Supabase credentials (project already created: `obqcjabipeixuyjgeqab`)
2. Add `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` to `src/backend/.env`
3. Create admin user in Supabase dashboard
4. Frontend-CMS uses `Authorization: Bearer <token>` header for write operations
5. Frontend-CRAM remains public (read-only access)
...
1. `npm install`
2. Copy `src/backend/.env.example` → `.env` and add Supabase credentials
